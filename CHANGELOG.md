...
### Security
- **Outbound provider requests no longer follow redirects** — the AI chat, embedding, and health-check requests followed HTTP redirects, which is an SSRF bypass (a 30x from an allowed host to a private/cloud-metadata IP defeats the host check) and a secret-leak vector (Guzzle replays custom auth headers such as Anthropic's `x-api-key` to the redirect target). Redirects are now disabled on all of them; provider API endpoints never legitimately redirect. (Extending the private-IP guard to cover chat/embedding, not just the health ping, remains a deliberate follow-up — all provider URLs are server-configured, not user input.)
- **Tool execution errors are no longer leaked to the model (and the user)** — when an orchestrator tool threw, the raw exception message was fed back to the model as the tool result; a `QueryException` carries SQL, table names, and sometimes connection credentials, which the model would then echo into its reply. The model now receives only a generic "tool failed to execute" message; the real exception stays in the log. Covered by `OrchestratorTest`.
- **The admin conversation viewer no longer renders unsanitized markup if the sanitizer fails to load** — `renderMarkdown()` fell back to raw `marked` output when DOMPurify was undefined, so a failed DOMPurify CDN load (SRI mismatch, proxy, offline) would render stored user/AI messages as raw HTML — a stored-XSS surface. It now falls back to escaped plain text unless both `marked` and `DOMPurify` are present, and the conversation-row `aria-label` (which interpolated the client-supplied `thread_id`) is now escaped.
- **The provider health-check badge no longer renders provider text as HTML** — the admin "Test" button built the offline status badge by concatenating the provider's `message`/`code` into `innerHTML`, so a hostile or MITM'd (plain-HTTP local) provider returning `<img src=x onerror=…>` would execute script in the admin dashboard. The badge is now built with `textContent`. 
...
### Fixed
- **The default zero-config stack (local Ollama + SSRF protection) is no longer a dead widget** — the SSRF guard blocked loopback addresses, so with the shipped defaults (`ollama` on `localhost` + `health_check` + `ssrf_protection`) the health check always failed and the chat widget wouldn't open. The guard now exempts loopback (`127.0.0.0/8`, `::1`) — a loopback provider URL is always operator-configured, not an SSRF pivot — while still blocking private ranges and cloud-metadata addresses. Covered by `HealthCheckTest`.
- **The orchestrator no longer discards its work when it hits the step limit** — when the tool-calling loop reached `orchestrator_max_steps` without the model volunteering a final answer, it threw an `O01` error, so the user paid for several model and tool calls and got an error bubble. It now makes one final model call with no tools offered, forcing a text answer from the results already gathered; only a genuinely empty answer is treated as an error. Covered by updated `OrchestratorTest`.
- **Conversation history is now safe under concurrent writes** — two requests on the same thread (two tabs, or a double-submit) could lose a reply: the DB driver decided which messages were "new" by counting, which breaks when each request holds a different snapshot, and creating a brand-new thread could throw a duplicate-key error under a race. Persistence is now append-only — each turn saves only its own user+assistant messages via a new `appendMessages()` repository method — and thread creation tolerates a concurrent creator by re-fetching. Covered by new `DatabaseMemoryTest` cases.
- **CI now tests the full claimed Laravel range** — the test workflow only ran against Laravel 12 while the package declares support for `^10.0|^11.0|^12.0`. The matrix now covers Laravel 10, 11, and 12 (each with the matching Testbench and PHPUnit versions across PHP 8.2–8.4) plus a `composer validate --strict` job.
- **Dev/test files no longer ship in the distributed package** — `.phpunit.result.cache` was tracked in git (committed before it was gitignored) and shipped to every install; it is now untracked, and a `.gitattributes` marks `.github/`, `tests/`, `temp/`, `graphify-out/`, `phpunit.xml`, and the docs/dotfiles as `export-ignore` so composer dist archives exclude them.
- **Published assets are cache-busted with the real package version** — the `?v=` query on the widget's CSS/JS was built from `config('app.version')`, which is not a standard Laravel key and resolved to an empty string, so browsers kept serving the old cached assets after a package upgrade. The cache-buster now uses the installed Composer package version (with a shipped constant fallback). Covered by `AssetVersionTest`
- **Documented env vars are now actually read by the config** — `AI_CHATBOX_LANGUAGE`, `AI_CHATBOX_MARKDOWN`, `AI_CHATBOX_HISTORY_LIMIT`, and `AI_CHATBOX_PRUNE_DAYS` were listed in the README but hardcoded in `config/ai-chatbox.php`, so setting them had no effect. Each now reads its env var (with the same default). The README also gained the missing `AI_CHATBOX_ORCHESTRATOR_MAX_TOKENS` row. (`AI_CHATBOX_STORAGE` was wired in the storage-default change above.)
- **Clearing or starting a new conversation mid-stream no longer breaks the widget** — all three chat drivers (Vue, Blade, Livewire) wrote streamed tokens into a captured message index, so pressing Clear or New while a reply was streaming emptied the list and the next token threw (leaving a broken render or a phantom bubble). Each driver now aborts the in-flight request (`AbortController`) on clear/new and guards its async callbacks so they stop cleanly once the conversation changes; the Vue driver also gained the missing double-send guard. The Vue asset bundle was rebuilt.
- **RAG embedding is batched, and `ai-chatbox:graphify` skips unchanged files** — ingestion made one embedding HTTP request per chunk, and the graphify importer deleted and re-embedded the entire corpus on every run. Chunks are now embedded in batches via the `/embeddings` array `input` (config `rag_embedding_batch_size`, default 32, with per-chunk fallback for endpoints that don't support arrays), and each document records a `content_hash` so `ai-chatbox:graphify` keeps unchanged files, re-ingests only changed/new ones, and removes documents whose source file is gone. Covered by `RagIngestorTest` and `GraphifyImportTest`. **Note:** the new `content_hash` column ships as migration `…000005` — run `php artisan migrate` after upgrading.
- **Knowledge-base re-ingestion is now atomic** — reprocessing a document deleted all its existing chunks *before* the minutes-long embedding phase, so a crash mid-embed left the document stuck `processing` with half its chunks gone and nothing to repair it. Ingestion now embeds into a buffer first (leaving the old chunks untouched), then swaps chunks and updates the document in a single transaction, and batch-inserts the new chunks instead of one query each. A crash during embedding now leaves the previous version fully intact. Covered by `RagIngestorTest`
- **Document chunking is now multibyte-safe** — the RAG chunker measured and sliced text by bytes, so a knowledge-base document containing non-ASCII text (CJK, accented characters, emoji) could have its overlap carry-over cut in the middle of a UTF-8 character. The resulting invalid byte sequence made the `utf8mb4` insert fail ("Incorrect string value") or the embedding request's JSON encoder throw, so the whole document failed to ingest. Sizing and slicing are now character-based (`mb_*`); a delimiter-less segment longer than one chunk is hard-wrapped on character boundaries; and the BOM strip no longer eats a legitimate leading character. ASCII documents chunk exactly as before. Covered by `DocumentChunkerTest`
...
### Changed
- **RAG ingestion is now bounded to reduce worker exhaustion** — document upload/reprocess runs synchronously in the request, and `rag_processing_timeout` defaulted to `0` (unlimited), so a large file could pin a PHP-FPM worker indefinitely. The default is now **300 seconds** (set `AI_CHATBOX_RAG_PROCESSING_TIMEOUT=0` to restore unlimited), and a new `rag_max_chunks_per_document` cap (default 5000, `AI_CHATBOX_RAG_MAX_CHUNKS`) drops excess chunks with a note so one oversized upload can't spawn tens of thousands of embedding calls. (Moving ingestion to a queued job remains a recommended follow-up.)
- **Browser chat history now defaults to `sessionStorage` instead of `localStorage`** — the default `storage` is now `session` (cleared when the tab closes) so a chat transcript is not left behind on a shared machine, and the value is env-driven via `AI_CHATBOX_STORAGE`. Set `AI_CHATBOX_STORAGE=local` to restore cross-session persistence. **This changes the shipped default** — history no longer persists across browser restarts unless you opt back in.
