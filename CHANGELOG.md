...
- **PDF and DOCX upload support for the Knowledge Base** — the RAG document uploader now accepts `.pdf` and `.docx` in addition to `.txt`/`.md`. New `DocumentTextExtractor` service converts each upload to plain text at index time (stored on the document, so reprocess never re-parses the binary):
  - `.docx` is parsed from `word/document.xml` using PHP's bundled `zip` extension — no extra dependency
  - `.pdf` is parsed via the **optional** `smalot/pdfparser` package (listed under `suggest`); if it isn't installed, PDF uploads return a clear "run `composer require smalot/pdfparser`" message while `.txt`/`.md`/`.docx` keep working
  - Upload validation switched from `mimes:txt,md` to a max-size check plus an extension whitelist (`DocumentTextExtractor::SUPPORTED`), since DOCX's zip-based content is frequently mis-detected by content-based MIME guessing
  - Knowledge Base UI updated — file picker `accept` and helper text now list all four formats
  - Scanned/image-only PDFs (no text layer) are not supported — OCR is out of scope
- **`rag_embedding_provider` config key — dedicated embedding provider** — lets RAG embeddings be sourced from a named provider different to the active chat provider, so providers without an embeddings API (notably **Anthropic**) can still use RAG. New `AiManager::embeddingConfig()` resolves the embedding URL, model, **and API token** from the named provider (e.g. chat with `anthropic`, embed with `ollama`/`openai`); both index-time (`RagController::processDocument()`) and retrieval-time (`PromptBuilder::ragContext()`) now route through it. When unset, the active provider's own embedding settings are used (unchanged). This also fixes the embedding call authenticating with the wrong key — it now uses the embedding provider's own `api_token` (e.g. your OpenAI key) instead of the chat provider's. Env: `AI_CHATBOX_EMBEDDING_PROVIDER`
